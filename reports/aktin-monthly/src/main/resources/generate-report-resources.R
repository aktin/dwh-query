...
#  dim(output) <- c(3,3)
#  report.table(output,'discharge.d.xml'))
#}, silent=FALSE)
...
try({
  ...
  #c[,3] <- sprintf(fmt="%.1f",c[,3])
  c[,3] <- paste(c[,3],'%')
  report.table(c,name='cedis.xml',align=c('left','right','right'),widths=c(60,15,15))
}, silent=FALSE)
...
try({
  ...
  #c[,3] <- sprintf(fmt="%.1f",c[,3])
  c[,3] <- paste(c[,3],'%')
  report.table(c,name='icd.xml',align=c('left','right','right'),widths=c(70,15,15))
