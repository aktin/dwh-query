...
try(
  {
    ...
    graph <- ggplot(data = data_frame, aes(x = description, y = time)) +
    ...
    report_svg(graph, "phys.d.box_test")
    rm(graph)

    graph <- ggplot(data_frame, aes(x = time)) +
      geom_histogram(
    ...
    report_svg(graph, "phys.d.hist_test")
    rm(data_frame, graph)
  },
  silent = FALSE
)
