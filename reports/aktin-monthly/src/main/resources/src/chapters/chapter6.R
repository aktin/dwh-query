...
try({
  ...
  b <- data.frame(Kategorie=factors$Discharge[!is.na(factors$Discharge)], Anzahl=format_number(discharge_table), Anteil=format_number((discharge_table / sum(discharge_table))*100,digits = 1))
  c <- rbind(b, data.frame(Kategorie="Summe",Anzahl=format_number(sum(discharge_table)),Anteil=format_number(100,digits=1)))
  #c[,3] <- sprintf(fmt="%.1f",c[,3])
  c[,3] <- paste(c[,3],'%')
  report_table(c,name='discharge.xml',align=c('left','right','right'),widths=c(30,15,15))
}, silent=FALSE)